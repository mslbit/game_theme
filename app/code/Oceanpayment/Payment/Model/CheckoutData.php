    ...
    private function buildBillingFields(array $billingAddr, bool $isVirtual, $quote, string $clientIp): array
    {
        ...
        if ($isVirtual) {
            ...
            return [
                ...
                'lastName'  => $lastName,
                'email'     => $customerEmail,
                'phone'     => isset($billingAddr['telephone']) ? $billingAddr['telephone'] : \Folix\SimpleCheckout\Model\BillingConfigProvider::generateMaskedMobileNumber(),
                'country'   => isset($billingAddr['country_id']) ? $billingAddr['country_id'] :( $this->geoIpService->getCountryCode($clientIp) ?: self::DEFAULT_COUNTRY),
