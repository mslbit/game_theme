...
namespace MslPay\LianLian\Model\Order\Payment\State;
 ...
 * 1. 下单时不发邮件：setCanSendNewEmailFlag(false)
 *    收银台/iframe 模式在下单时支付尚未经后端确认，不应发确认邮件，
 *    等异步通知确认支付成功后再发。
 * 2. 待支付场景下状态改为 PENDING_PAYMENT：
 *    原生 AuthorizeCommand 在 isTransactionPending=true 时设 STATE_PAYMENT_REVIEW，
 *    但收银台/3D验证场景下订单实际是"待支付"而非"支付审查"，
...
class AuthorizeCommand extends MagentoAuthorizeCommand
{
     ...
     * 连连支付方式代码列表
     */
    private const LIANLIAN_METHODS = [
        'mslpay_lianlian_checkout',
        'mslpay_lianlian_iframe',
    ];
     ...
     * 连连支付方式：
     * - 抑制邮件通知（等异步通知确认后再发）
     * - isTransactionPending=true（收银台/3D验证）→ STATE_PENDING_PAYMENT
    ...
    {
        if (!$this->isLianLianMethod($payment)) {
        ...
        if ($payment->getIsFraudDetected()) {
            ...
            /*
             * 收银台/3D验证场景：PENDING_PAYMENT 代替 PAYMENT_REVIEW
             *
             * 原生逻辑在此处设 STATE_PAYMENT_REVIEW，但：
             * - 收银台跳转/3D验证是正常支付流程，不是风控审查
     ...
     * 检查是否为连连支付方式
     */
    private function isLianLianMethod(OrderPaymentInterface $payment): bool
    {
        return in_array($payment->getMethod(), self::LIANLIAN_METHODS, true);
    }
}